...
/* Formal-test hints: pedagogical, question-sensitive, cost 5 inzichtpunten. */
(function () {
  const HINT_COST = 5;
  const HINT_USED = {};
  ...
  function specificHint(q) {
    ...
    if (/klein naar groot|groot naar klein|rangschik/i.test(p)) return "Vergelijk de getallen systematisch. Kijk eerst naar de grootste plaatswaarde en controleer daarna de volledige volgorde.";
    ...
    if (type === "symbol") return "Vergelijk beide waarden rechtstreeks. Verder naar rechts op de getallenlijn betekent groter.";
    ...
    if (/vereenvoudig|vereenvoudigde vorm/i.test(p)) return "Zoek een gemeenschappelijke deler van teller en noemer en deel beide door dezelfde waarde.";
    ...
    if (/verhouding|evenredig|recept|schaal|personen/i.test(p + t)) return "Zoek eerst de vermenigvuldigingsfactor tussen de bekende waarden. Pas dezelfde factor toe op de andere grootheid.";
    ...
    if (/deler|deelbaar/i.test(p)) return "Een deler deelt het getal zonder rest. Gebruik bij deelbaarheid de passende regel.";
    if (/priem|factorisatie|priemfactor/i.test(p) || type === "factorization") return "Begin met de kleinste priemgetallen. Deel telkens zolang dat kan en noteer de priemfactoren.";
    if (/GGD/i.test(p)) return "De GGD is de grootste positieve deler die beide getallen gemeen hebben.";
    if (/KGV/i.test(p)) return "Het KGV is het kleinste positieve gemeenschappelijke veelvoud.";
    if (/macht|exponent|²|³|⁴/.test(p)) return "Een exponent geeft aan hoe vaak het grondtal als factor voorkomt.";
    if (/wortel|√/.test(p)) return "Zoek het getal waarvan het kwadraat de waarde onder de wortel is.";
    if (/irrationaal/i.test(p)) return "Een rationaal getal kan als breuk van twee gehele getallen worden geschreven.";
    ...
    if (type === "ordering") return "Vergelijk de opties paarsgewijs en plaats ze één voor één in de gevraagde volgorde.";
    if (type === "select") return "Beoordeel elke optie afzonderlijk aan de hand van de eigenschap in de vraag.";
    if (type === "choice") return "Elimineer antwoordmogelijkheden die strijdig zijn met de definitie of rekenregel.";
  ...
  window.examQuestionHTML = function (q, i) {
    ...
    return html.replace(
      ...
      '<div class="exam-hint-actions"><button type="button" class="btn exam-hint-btn" data-exam-hint="' + esc(q.id) + '">Hint (5 inzicht)</button><span class="exam-hint-text" data-exam-hint-text="' + esc(q.id) + '"></span></div><div class="exam-feedback"></div>'
    );
  };
  ...
  document.addEventListener("click", function (e) {
    ...
    let q = null;
    if (typeof currentExamQuestions === "function" && typeof parseHash === "function") {
      const parts = parseHash();
      const phase = parts[1] || 1;
      const mid = parts[3] || "";
      q = currentExamQuestions("mile", phase, mid).find((x) => x.id === qid) || null;
    }
    if (!q) return;
    HINT_USED[qid] = true;
